...
function blockChildren(blocks, color, refs) {
    const out = [];
...
function logoRun(business, pro) {
    if (!pro || !business.logo_path || !existsSync(business.logo_path))
        return null;
...
export async function buildDocx(o) {
    ...
    if (biz.name && style !== "plain") {
        if (logo)
            head.push(new Paragraph({ children: [logo], alignment: AlignmentType.RIGHT }));
    ...
    if (style === "letter") {
        if (o.date)
            head.push(new Paragraph({ text: o.date, alignment: AlignmentType.RIGHT, spacing: { after: 200 } }));
        if (o.recipient)
            head.push(textPara(o.recipient, { spacing: { after: 240 } }));
        ...
        if (style === "proposal" && (o.recipient || o.date)) {
            head.push(textPara([o.recipient ? `Prepared for ${o.recipient}` : "", o.date ?? ""].filter(Boolean).join("   |   "), { spacing: { after: 280 } }));
        }
    }
    const numberRefs = [];
    const bodyChildren = blockChildren(o.blocks, color, numberRefs);
    if (!numberRefs.length)
        numberRefs.push(NUM_REF);
...
export function toHtml(title, blocks) {
    const body = [];
    for (const b of blocks) {
        if (b.type === "heading")
            body.push(`<h${Math.min(6, b.level)}>${inlineHtml(b.text)}</h${Math.min(6, b.level)}>`);
        else if (b.type === "para")
            body.push(`<p>${inlineHtml(b.text)}</p>`);
        else if (b.type === "code")
            body.push(`<pre>${esc(b.text)}</pre>`);
        else if (b.type === "bullets")
            body.push(listHtml(b));
        else {
            body.push(`<table><thead><tr>${b.headers.map((h) => `<th>${inlineHtml(h)}</th>`).join("")}</tr></thead>` +
