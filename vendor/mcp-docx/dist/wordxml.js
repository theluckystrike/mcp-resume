...
function element(xml, name, from) {
    ...
    if (!m || m.index !== from)
        return null;
    if (m[2])
        return { inner: "", end: from + m[0].length };
    ...
    while (depth > 0) {
        ...
        if (close < 0)
            return { inner: xml.slice(start), end: xml.length };
        openTag.lastIndex = i;
        let nested;
        let nextOpen = -1;
        while ((nested = openTag.exec(xml))) {
            if (nested.index < close) {
                nextOpen = nested.index;
                break;
            }
            else
                break;
        ...
        if (depth === 0)
            return { inner: xml.slice(start, close), end: i };
    }
    return null;
}
function headingLevel(inner) {
...
export function numberingFormats(numberingXml) {
...
function listInfo(inner, fmts) {
    ...
    if (numId && fmts?.has(numId))
        ordered = fmts.get(numId) !== "bullet";
...
export function blocksOf(xml, fmts) {
    const out = [];
    let lastNumId = "";
    let i = 0;
    while (i < xml.length) {
        let p = xml.indexOf("<w:p", i);
        while (p >= 0 && !/^<w:p[\s/>]/.test(xml.slice(p, p + 5)))
            p = xml.indexOf("<w:p", p + 4);
        const t = xml.indexOf("<w:tbl", i);
        const pOk = p >= 0;
        const next = pOk && (t < 0 || p < t) ? { at: p, kind: "p" } : t >= 0 ? { at: t, kind: "tbl" } : null;
        if (!next)
            break;
        if (next.kind === "tbl") {
        ...
        if (lvl) {
            out.push({ type: "heading", level: lvl, text: text.trim() });
            continue;
        }
        ...
        if (li.list) {
            ...
            if (prev && prev.type === "bullets" && prev.ordered === li.ordered && lastNumId === li.numId) {
                ...
                out.push({ type: "bullets", items: [text.trim()], ordered: li.ordered, levels: [li.level] });
            lastNumId = li.numId;
            continue;
        }
        lastNumId = "";
 ...
 */
export function assertDocx(buf, path) {
    const entries = readZip(buf);
    if (!entries.some((e) => e.name === "word/document.xml")) {
        throw new Error(`${path} is a ZIP but has no word/document.xml, so it is not a Word .docx; nothing was written.`);
    }
}
export function documentXml(buf) {
    const entries = readZip(buf);
    const doc = entries.find((e) => e.name === "word/document.xml");
    if (!doc)
        throw new Error("this file has no word/document.xml, so it is not a Word .docx (a .doc or .rtf will not work).");
 ...
 */
export function stripInvalidXml(s) {
    let out = "";
    let removed = 0;
...
export function placeholdersIn(buf) {
    const seen = [];
    for (const e of readZip(buf)) {
        if (!/^word\/(document|header\d*|footer\d*)\.xml$/.test(e.name))
            continue;
...
function textNodes(inner) {
    ...
    while ((m = NODE.exec(inner))) {
        ...
        let attrs = "";
        if (m[3] !== undefined) {
            text = unescapeXml(m[3]);
            attrs = m[2] ?? "";
        }
        else if (m[0].startsWith("<w:t/") || m[0].startsWith("<w:t ")) {
            text = "";
            attrs = m[1] ?? "";
        }
        else {
            editable = false;
            text = m[0].startsWith("<w:tab") ? "\t" : "\n";
        }
...
export function paragraphText(inner) {
 ...
 */
function applyEdits(inner, edits) {
    const nodes = textNodes(inner);
    if (!nodes.some((n) => n.editable))
        return null;
    for (const e of edits) {
        if (nodes.some((n) => !n.editable && e.start < n.to && e.end > n.from))
            return null;
    }
    let out = "";
    let copied = 0;
    for (const n of nodes) {
        ...
        for (let i = n.from; i < n.to; i++) {
            for (const e of hits)
                if (e.start === i)
                    text += e.value; // insert where the placeholder began
            if (hits.some((e) => i >= e.start && i < e.end))
                continue; // drop the placeholder characters
...
function forEachParagraph(xml, fn) {
    let out = "";
    ...
    while (search < xml.length) {
        ...
        if (!/^<w:p[\s/>]/.test(xml.slice(at, at + 5))) {
            search = at + 4;
            continue;
        }
        ...
        if (edits && edits.length)
            rewritten = applyEdits(inner, edits);
        out += rewritten ?? inner;
