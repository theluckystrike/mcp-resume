...
import { buildDocx, toHtml } from "./build.js";
import { parseMarkdown } from "./md.js";
import { addDoc, dataDir, docsInMonth, getBusiness, getDocs, hasBusiness, nextNumber, setBusiness, updateDoc, } from "./store.js";
...
const NO_BUSINESS_NOTE = "No business profile yet: the document shows a placeholder sender. " +
...
function businessMissing() { return !hasBusiness() || !getBusiness().name.trim(); }
function issuer() {
    const b = getBusiness();
 ...
 */
function record(kind, title, path, client, number, data) {
    try {
...
const server = new McpServer({ name: "mcp-docx", version: VERSION }, { capabilities: { tools: {}, resources: {}, prompts: {} } });
/* ------------------------------------------------------------------ business */
server.registerTool("business_set", {
    ...
    try {
        return await locked(() => {
            ...
            setBusiness(biz);
            if (a.phone || a.timezone)
                writeSharedProfile({ phone: a.phone, timezone: a.timezone });
...
function sectionBlocks(sections) {
    ...
    for (const s of sections) {
        if (s.heading)
            blocks.push({ type: "heading", level: s.level ?? 2, text: s.heading });
        for (const p of s.paragraphs ?? [])
            blocks.push({ type: "para", text: p });
        if (s.bullets?.length)
            blocks.push({ type: "bullets", items: s.bullets, ordered: false });
        if (s.numbered?.length)
            blocks.push({ type: "bullets", items: s.numbered, ordered: true });
        if (s.table)
            blocks.push({ type: "table", headers: s.table.headers, rows: s.table.rows });
    }
    return blocks;
}
/**
...
function cleanForXml(value, count) {
    ...
    if (Array.isArray(value))
        return value.map((v) => cleanForXml(v, count));
    if (value && typeof value === "object") {
        const out = {};
        for (const [k, v] of Object.entries(value))
            out[k] = cleanForXml(v, count);
        return out;
    }
    return value;
}
...
async function writeDoc(title, blocks, style, out, kind, opts = {}) {
    ...
    for (const b of clean.blocks) {
        if (b.type === "para" && /\n\n/.test(b.text)) {
            for (const part of b.text.split(/\n{2,}/))
                if (part.trim())
                    split.push({ type: "para", text: part });
        }
        else
            split.push(b);
    }
...
server.registerTool("doc_create", {
    ...
    try {
        ...
        if (!blocks.length)
            return fail("a document needs at least one section with a heading, paragraphs, bullets or a table.");
        const style = (a.style ?? "plain");
...
server.registerTool("doc_from_markdown", {
    ...
    try {
        ...
        if (!blocks.length)
            return fail("the markdown is empty.");
        let title = a.title;
        let body = blocks;
        if (!title) {
        ...
        const res = await locked(() => writeDoc(title, body, (a.style ?? "plain"), a.out_path, "markdown", { overwrite: a.overwrite }));
        const counts = body.reduce((acc, b) => { acc[b.type] = (acc[b.type] ?? 0) + 1; return acc; }, {});
...
server.registerTool("doc_read", {
    ...
    try {
        ...
        if (!existsSync(p))
            return fail(`no file at ${p}.`);
        if (!/\.docx$/i.test(p))
            return fail(`${p} is not a .docx file. Legacy .doc and .rtf are not readable here.`);
        const blocks = readDocx(readFileSync(p));
        if (a.format === "json")
            return json({ path: p, blocks });
        const headings = blocks.filter((b) => b.type === "heading")
            .map((b) => `${"  ".repeat(b.level - 1)}${b.text}`);
        const text = blocks.map(blockText).join("\n\n");
        return ok(`${basename(p)}: ${blocks.length} blocks, ${text.length} characters.\n\n` +
...
server.registerTool("doc_fill_template", {
    ...
    try {
        ...
        if (!existsSync(tpl))
            return fail(`no template at ${tpl}.`);
...
function proposalBody(a) {
    ...
    if (biz.default_tax_rate)
        blocks.push({ type: "para", text: `All amounts exclude VAT at ${biz.default_tax_rate}%.` });
    blocks.push({ type: "heading", level: 2, text: "Acceptance" });
    blocks.push({ type: "para", text: `This proposal is valid until ${a.valid_until ?? "the date agreed in writing"}. ` +
...
server.registerTool("proposal_create", {
    ...
    try {
        ...
        if (gated)
            return ok(gated);
        if (businessMissing())
            return fail(NO_BUSINESS_NOTE + " Nothing was written and no reference was used.");
        ...
        const out = await locked(async () => {
            ...
            const w = await writeDoc(a.project_title, blocks, "proposal", a.out_path, "proposal", { client: a.client, number, date: `${isoDate()} | Ref ${number}`, overwrite: a.overwrite, data: a });
        ...
        const omittedNote = omitted.length
            ...
            : "";
        return ok(`Created proposal ${out.number} for ${a.client}. The letterhead came from your business_set profile, ` +
...
server.registerTool("proposal_update", {
    ...
    try {
        ...
        const rec = getDocs().find((d) => d.kind === "proposal" && d.number === ref);
        if (!rec)
            return fail(`no proposal ${ref} in the document history. The docs://recent resource lists the references.`);
        if (!rec.data) {
            return fail(`proposal ${ref} was created before proposals were stored as structured data, so it cannot be ` +
                `rewritten. Create a new one with proposal_create.`);
        }
        const patch = {};
        for (const k of ["client", "project_title", "summary", "scope", "deliverables", "timeline", "price", "valid_until"]) {
            const v = a[k];
            if (v !== undefined)
                patch[k] = v;
        }
        ...
        const omittedNote = omitted.length
            ...
            : "";
        const out = await locked(() => writeDoc(merged.project_title, blocks, "proposal", rec.path, "proposal", {
            client: merged.client, number: ref, date: `${isoDate()} | Ref ${ref}`,
            overwrite: true, data: merged, replaceId: rec.id,
        }));
        return ok(`Updated proposal ${ref} in place; the reference and the file are unchanged. Every field you did not ` +
...
server.registerTool("contract_create", {
    ...
    try {
        ...
        const out = await locked(async () => {
            ...
            const w = await writeDoc(`Service Agreement - ${a.client}`, blocks, "proposal", a.out_path, "contract", { client: a.client, number, date: `${isoDate()} | Ref ${number}`, overwrite: a.overwrite, data: a });
            return { path: w.path, note: w.note, number };
        });
        return ok(`Created service agreement ${out.number} for ${a.client}.\n\n` +
...
}, ({ client, project, hours, rate }) => {
    const biz = getBusiness();
    const text = `Write a proposal for ${client}${project ? ` for "${project}"` : ""}.\n\n` +
        (hours || rate
